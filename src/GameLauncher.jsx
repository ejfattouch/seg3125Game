...
const GameLauncher = () => {
  ...
  const startGame = (e) => {
    ...
    const wordList = data.wordList.map((word) => ({
    ...
    <div className="min-h-screen text-center text-white flex flex-col gap-4 space-y-6 mx-4 py-12 px-4 sm:px-0">
      <div className="flex flex-col gap-4">
        ...
        <p className={"text-lg max-w-xl mx-auto"}>
          Challenge your mind and seek hidden words. Choose your difficulty
          level and start your word-hunting adventure!
        </p>
      </div>
      <div>
        <h1 className={"text-2xl font-bold"}>Choose Your Challenge</h1>

        <div className="grid grid-cols-1 gap-10 lg:px-0 lg:grid-cols-3 mt-6 mb-10 lg:mb-20">
          {dataArray.map((data_set, index) => (
            <Card
              key={index}
              onClick={(e) => {
                handleGameSelect(e, index);
              }}
              className={cn(
                selectedDifficulty[index]
                  ? "border-primary-purple"
                  : "border-gray-600",
                "bg-gray-700 border cursor-pointer rounded-xl shadow-md"
              )}
            >
              <CardHeader className={"select-none"}>
                ...
                <CardDescription>
                  <p
                    className={cn(
                      "rounded-full w-fit px-3 mx-auto",
                      data_set.hint_bg,
                      data_set.hint_text_color
                    )}
                  >
              ...
              <CardContent className={"pb-1"}>
                <h1 className={"font-bold text-md select-none"}>
                  Available Letters:
                </h1>
                <div
                  className={
                    "flex flex-row gap-2 flex-wrap justify-center mt-3"
                  }
                >
        ...
        <Button
          className={cn(
            data_select === null
              ...
              : "bg-primary-purple cursor-pointer",
            "text-2xl py-6 px-10 cursor-pointer mb-10 lg:mb-0"
          )}
